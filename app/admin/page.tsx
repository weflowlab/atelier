...
type Tab = "inquiries" | "faqs" | "analytics" | "traffic";
...
const CONTENT_TABS: Tab[] = ["faqs", "analytics", "traffic"];
...
export default function AdminPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("atl_admin_tab") as Tab | null;
    if (saved && TABS.some((t) => t.key === saved)) setTab(saved);
  }, []);
  useEffect(() => {
    localStorage.setItem("atl_admin_tab", tab);
  }, [tab]);
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const [iData, fData] = await Promise.all([iRes.json(), fRes.json()]);
      ...
      >
        <div
          style={{
            ...
            width: "100%",
            maxWidth: "440px",
            boxShadow: "0 8px 40px rgba(43,37,33,0.15)",
          }}
        >
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          background: "var(--surface)",
          boxShadow: "4px 0 32px rgba(43,37,33,0.2)",
          display: "none",
          flexDirection: "column",
