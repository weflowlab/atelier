...
export default function ProductLightbox({ item, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* 패널 */}
      <div className="relative z-10 flex w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-background text-foreground shadow-2xl lg:flex-row">
        {/* 닫기 버튼 */}
        <button
          type="button"
          aria-label="닫기"
            ...
            <div className="flex gap-2 overflow-x-auto p-3 no-scrollbar">
              {photos.map((_, i) => (
                <button
                  ...
                  aria-label={`${i + 1}번 사진`}
                  onClick={() => setIdx(i)}
                  className={`h-14 w-20 shrink-0 overflow-hidden rounded-md border-2 transition ${i === idx ? "border-gold" : "border-transparent opacity-60 hover:opacity-100"}`}
                >
                  {photos[i] ? (
        ...
        {/* 우: 제품 정보 + CTA */}
        <div className="flex flex-col justify-between gap-6 p-6 sm:p-8 lg:w-2/5">
          <div>
            <p className="eyebrow">PRODUCT</p>
            <h3 className="serif mt-2 text-2xl font-semibold md:text-3xl">{item.name}</h3>
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <a
              href="#gallery"
              onClick={onClose}
              className="inline-flex h-12 items-center justify-center rounded-full border border-line px-6 text-sm font-medium transition hover:border-accent hover:text-accent"
            >
              시공 사례 더 보기
            </a>
          </div>
